...
from dflow.models.geometry import construct_3d_basis, global_to_local, get_backbone_dihedral_angles
...
class NodeEmbedder(nn.Module):
    ...
    def skip_forward(self, aa):
        N, L = aa.size()
        aa_feat = self.aatype_embed(aa)  # (N, L, feat)
        placeholder_feat = torch.zeros(N, L, (self.max_aa_types * self.max_num_atoms * 3) + self.dihed_embed.get_out_dim(3), device=aa.device)
        out_feat = self.mlp(torch.cat([aa_feat, placeholder_feat], dim=-1))  # (N, L, F)
        return out_feat
