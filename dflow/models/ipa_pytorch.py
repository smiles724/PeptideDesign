...
def compute_angles(ca_pos, pts):
    ...
    phi_angles = all_atom.calculate_neighbor_angles(calpha_vecs.reshape(-1, 3), ipa_pts.reshape(-1, 3)).reshape(batch_size, num_res, num_res, num_heads, num_pts)
    return phi_angles
...
class EdgeTransition(nn.Module):
    def __init__(self, *, node_embed_size, edge_embed_in, edge_embed_out, num_layers=2, node_dilation=2):
    ...
    def forward(self, node_embed, edge_embed):
        ...
        edge_bias = torch.cat([torch.tile(node_embed[:, :, None, :], (1, 1, num_res, 1)), torch.tile(node_embed[:, None, :, :], (1, num_res, 1, 1)), ], axis=-1)
...
class InvariantPointAttention(nn.Module):
    ...
    def forward(self, s: torch.Tensor, z: Optional[torch.Tensor], r: Rigid, mask: torch.Tensor, attention_mask: torch.Tensor = None, _offload_inference: bool = False,
                _z_reference_list: Optional[Sequence[torch.Tensor]] = None, ) -> torch.Tensor:
        """
        Args:
            s:
        ...
        # [*, H, N_res, N_res]
        a = torch.matmul(permute_final_dims(q, (1, 0, 2)),  # [*, H, N_res, C_hidden]
...
class TorsionAngles(nn.Module):
    ...
    def forward(self, s):
        ...
        norm_denom = torch.sqrt(torch.clamp(torch.sum(unnormalized_s ** 2, dim=-1, keepdim=True), min=self.eps, ))
