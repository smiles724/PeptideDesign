...
from dflow.models.geometry import angstrom_to_nm, pairwise_dihedrals
from dflow.models.utils import AngularEncoding
from dflow.data.pep_constants import BBHeavyAtom, AA
...
class EdgeEmbedder(nn.Module):
    ...
    def __init__(self, feat_dim, max_num_atoms, max_aa_types=22, max_relpos=32, num_bins=16):
        ...
        self.distance_embed = nn.Sequential(nn.Linear(max_num_atoms * max_num_atoms, feat_dim), nn.ReLU(), nn.Linear(feat_dim, feat_dim), nn.ReLU(), )

        self.dihedral_embed = AngularEncoding()
        self.feat_dihed_dim = self.dihedral_embed.get_out_dim(2)  # Phi and Psi

        infeat_dim = feat_dim * 3 + self.feat_dihed_dim
        self.out_mlp = nn.Sequential(nn.Linear(infeat_dim, feat_dim), nn.ReLU(), nn.Linear(feat_dim, feat_dim), nn.ReLU(), nn.Linear(feat_dim, feat_dim), )

    def skip_forward(self, aa, res_nb, chain_nb):
        N, L = aa.size()
        ...
        relpos = torch.clamp(res_nb[:, :, None] - res_nb[:, None, :], min=-self.max_relpos, max=self.max_relpos, )  # (N, L, L)
        feat_relpos = self.relpos_embed(relpos + self.max_relpos) * same_chain[:, :, :, None]

        feat_all = torch.cat([feat_aapair, feat_relpos, torch.zeros(N, L, L, feat_aapair.shape[-1] + self.feat_dihed_dim, device=aa.device)], dim=-1)
    ...
    def forward(self, aa, res_nb, chain_nb, pos_atoms, mask_atoms, structure_mask=None, sequence_mask=None):
        ...
        # Distances
        d = angstrom_to_nm(torch.linalg.norm(pos_atoms[:, :, None, :, None] - pos_atoms[:, None, :, None, :], dim=-1, ord=2, )).reshape(N, L, L, -1)  # (N, L, L, A*A)
