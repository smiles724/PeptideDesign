...
def process_dic(state_dict):
    ...
    for k, v in state_dict.items():
        if k.startswith('model.'):   # controlNet
            new_state_dict[k[6:]] = v
        elif k.startswith("module."):     # DP module
...
def dataset_creation(dataset_class, cfg, task, all_angles):
    train_dataset = dataset_class(dataset_cfg=cfg, task=task, is_training=True, all_angles=all_angles)
    eval_dataset = dataset_class(dataset_cfg=cfg, task=task, is_training=False, all_angles=all_angles)
...
def log_losses(loss, loss_dict, scalar_dict, it, tag, ):
