...
def to_atom37(trans, rots):
    ...
    final_atom37 = compute_backbone(du.create_rigid(rots, trans), torch.zeros(num_batch, num_res, 2, device=trans.device))[0]
    return final_atom37


def torsion_angles_to_frames(r: Rigid, alpha: torch.Tensor, aatype: torch.Tensor, ):
...
def prot_to_torsion_angles(aatype, atom37, atom37_mask):
    ...
    prot_feats = {"aatype": aatype, "all_atom_positions": atom37, "all_atom_mask": atom37_mask, }
...
def frames_to_atom14_pos(r: Rigid,  # type: ignore [valid-type]
        aatype: torch.Tensor, ):
    ...
    with torch.no_grad():
        ...
        group_mask = torch.nn.functional.one_hot(group_mask, num_classes=DEFAULT_FRAMES.shape[-3], )
...
def compute_backbone(bb_rigids, psi_torsions):
    torsion_angles = torch.tile(psi_torsions[..., None, :], tuple([1 for _ in range(len(bb_rigids.shape))]) + (7, 1))
    aatype = torch.zeros(bb_rigids.shape, device=bb_rigids.device).long()
    all_frames = torsion_angles_to_frames(bb_rigids, torsion_angles, aatype, )
...
def atom37_from_trans_rot(trans, rots, res_mask=None):
    ...
    atom37 = compute_backbone(rigids, torch.zeros(trans.shape[0], trans.shape[1], 2, device=trans.device))[0]
    batch_atom37 = []
