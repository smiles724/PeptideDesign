...
from openfold.model.embedders import (InputEmbedder, RecyclingEmbedder, TemplateAngleEmbedder, TemplatePairEmbedder, ExtraMSAEmbedder, )
...
class AlphaFold(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.input_embedder = InputEmbedder(**config["input_embedder"], )
    ...
    def embed_templates(self, batch, z, pair_mask, templ_dim):
        ...
        for i in range(n_templ):
            ...
            single_template_feats = tensor_tree_map(lambda t: torch.index_select(t, templ_dim, idx), batch, )
            ...
            # [*, S_t, N, N, C_t]
            t = build_template_pair_feat(single_template_feats, inf=self.config.template.inf, eps=self.config.template.eps, **self.config.template.distogram, ).to(z.dtype)
        ...
        template_embeds = dict_multimap(partial(torch.cat, dim=templ_dim), template_embeds, )

        # [*, S_t, N, N, C_z]
        t = self.template_pair_stack(template_embeds["pair"], pair_mask.unsqueeze(-3).to(dtype=z.dtype), chunk_size=self.globals.chunk_size, _mask_trans=self.config._mask_trans, )

        # [*, N, N, C_z]
        t = self.template_pointwise_att(t, z, template_mask=batch["template_mask"].to(dtype=z.dtype), chunk_size=self.globals.chunk_size, )
    ...
    def iteration(self, feats, m_1_prev, z_prev, x_prev, _recycle=True):
        ...
        # m: [*, S_c, N, C_m]
        # z: [*, N, N, C_z]
        m, z = self.input_embedder(feats["target_feat"], feats["residue_index"], feats["msa_feat"], )
        ...
        if self.config.template.enabled:
            ...
            template_embeds = self.embed_templates(template_feats, z, pair_mask.to(dtype=z.dtype), no_batch_dims, )
        ...
        if self.config.extra_msa.enabled:
            ...
            # [*, N, N, C_z]
            z = self.extra_msa_stack(a, z, msa_mask=feats["extra_msa_mask"].to(dtype=a.dtype), chunk_size=self.globals.chunk_size, pair_mask=pair_mask.to(dtype=z.dtype),
        ...
        # z: [*, N, N, C_z]
        # s: [*, N, C_s]
        m, z, s = self.evoformer(m, z, msa_mask=msa_mask.to(dtype=m.dtype), pair_mask=pair_mask.to(dtype=z.dtype), chunk_size=self.globals.chunk_size,
                                 _mask_trans=self.config._mask_trans, )
        ...
        outputs["sm"] = self.structure_module(s, z, feats["aatype"], mask=feats["seq_mask"].to(dtype=s.dtype), )
    ...
    def _enable_activation_checkpointing(self):
        self.template_pair_stack.blocks_per_ckpt = (self.config.template.template_pair_stack.blocks_per_ckpt)
    ...
    def forward(self, batch):
        ...
        for cycle_no in range(num_iters):
            ...
            with torch.set_grad_enabled(is_grad_enabled and is_final_iter):
                ...
                outputs, m_1_prev, z_prev, x_prev = self.iteration(feats, m_1_prev, z_prev, x_prev, _recycle=(num_iters > 1))
