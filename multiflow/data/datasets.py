...
def _process_csv_row(processed_file_path, ):  # read from pkl
    ...
    chain_feats = data_transforms.atom37_to_torsion_angles_(bb_angles=True)(chain_feats)
...
class BaseDataset(Dataset):
    ...
    def process_csv_row(self, csv_row, ):
        ...
        processed_row = _process_csv_row(path)      # read processed .pkl file
...
def pdb_init_(self, *, dataset_cfg, is_training, task, ):
    self._log = logging.getLogger(__name__)
    self._is_training = is_training
    self._dataset_cfg = dataset_cfg
    self.task = task
