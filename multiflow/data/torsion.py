...
import multiflow.data.pep_constants as constants
from multiflow.models.geometry import *
...
def get_omega_angle(pos14: torch.Tensor, prev_pos14: torch.Tensor) -> torch.Tensor:
    return _get_torsion(prev_pos14[1], prev_pos14[2], pos14[0], pos14[1]).reshape([1])  # CA(-1), C(-1), N, CA


def get_phi_angle(pos14: torch.Tensor, prev_pos14: torch.Tensor) -> torch.Tensor:
    return _get_torsion(prev_pos14[2], pos14[0], pos14[1], pos14[2]).reshape([1])  # C(-1), N, CA,N C
...
def get_torsion_angle(pos14: torch.Tensor, aa: torch.LongTensor):
    ...
    for i in range(pos14.shape[0]):
        if aa[i] < constants.AA.UNK:  # 0-19
            if i >= 1:
                omega = get_omega_angle(pos14[i], pos14[i - 1])
                phi = get_phi_angle(pos14[i], pos14[i - 1])
            else:
                phi = omega = torch.tensor([(float('nan'))])
            psi = get_psi_angle(pos14[i])
            chi = get_chi_angles(aa[i].item(), pos14[i])
            torsion_this = torch.cat([omega, phi, psi, chi], dim=0)  # [omega, phi, psi, chi]
            torsion_mask_this = torsion_this.isfinite()
        else:  # UNK
            torsion_this = torch.full([7], 0.)
            torsion_mask_this = torch.full([7], False)
        torsion.append(torsion_this.nan_to_num(posinf=0.))  # transfer nan to 0.0?
...
def _get_rigid_group(aa: torch.Tensor) -> (torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor):
    ...
    atom14_position = constants.restype_heavyatom_rigid_group_positions.to(aa.device)[aa].reshape(batch_size, n_res, 14, 3)
...
def full_atom_reconstruction(R_bb: torch.Tensor, t_bb: torch.Tensor, angles: torch.Tensor, aa: torch.Tensor, ) -> (torch.Tensor, torch.Tensor, torch.Tensor):
    ...
    R_psi, t_psi = compose_chain([(R_bb, t_bb), (rigid_rotation[:, :, constants.PSI_FRAME], rigid_translation[:, :, constants.PSI_FRAME]), (rot_psi, zeros), ])
    R_chi1, t_chi1 = compose_chain([(R_bb, t_bb), (rigid_rotation[:, :, constants.CHI1_FRAME], rigid_translation[:, :, constants.CHI1_FRAME]), (rot_chi1, zeros), ])
    R_chi2, t_chi2 = compose_chain([(R_chi1, t_chi1), (rigid_rotation[:, :, constants.CHI2_FRAME], rigid_translation[:, :, constants.CHI2_FRAME]), (rot_chi2, zeros), ])
    R_chi3, t_chi3 = compose_chain([(R_chi2, t_chi2), (rigid_rotation[:, :, constants.CHI3_FRAME], rigid_translation[:, :, constants.CHI3_FRAME]), (rot_chi3, zeros), ])
    R_chi4, t_chi4 = compose_chain([(R_chi3, t_chi3), (rigid_rotation[:, :, constants.CHI4_FRAME], rigid_translation[:, :, constants.CHI4_FRAME]), (rot_chi4, zeros), ])
...
torsions_mask = torch.zeros([22, 7]).float()  # 0-19, X, PAD
for i in range(21):
    torsions_mask[i] = torch.tensor([True, True, True] + constants.chi_angles_mask[i]).float()   # TODO: mask for phi/omega
