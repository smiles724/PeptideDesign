...
torsions_mask_vocab = torch.zeros([21, 7]).float()  # 0-19, X
for i in range(20):
    torsions_mask_vocab[i] = torch.tensor([True, True, True] + rc.chi_angles_mask[i]).float()
...
class Interpolant:
    ...
    def __init__(self, cfg):
        ...
        print(self._aatypes_cfg.interpolant_type)
    ...
    def sample(self, num_batch, num_res, model, num_timesteps=None, trans_0=None, rotmats_0=None, aatypes_0=None, trans_1=None, rotmats_1=None, aatypes_1=None,
        ...
        if angles_0 is None:
            angles_0 = torus.tor_random_uniform((num_batch, num_res, 7), device=self._device, dtype=angles_1.dtype)  # (B,L,7)
        ...
        if angles_1 is None:
            angles_1 = torch.zeros(num_batch, num_res, 7, device=self._device)
