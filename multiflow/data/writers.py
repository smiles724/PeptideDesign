...
from multiflow.data.pep_constants import AA, restype_to_allatom_names, restype_to_heavyatom_names
...
def save_pdb(data, path=None):
    ...
    def _build_chain(builder, aa_ch, pos_heavyatom_ch, mask_heavyatom_ch, chain_id_ch, resseq_ch, icode_ch):
        ...
        for aa_res, pos_allatom_res, mask_allatom_res, resseq_res, icode_res in zip(aa_ch, pos_heavyatom_ch, mask_heavyatom_ch, resseq_ch, icode_ch):
            ...
            builder.init_residue(resname=str(restype), field=' ', resseq=resseq_res.item(), icode=icode_res, )
