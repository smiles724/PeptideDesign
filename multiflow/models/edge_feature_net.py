...
class EdgeFeatureNet(nn.Module):
    ...
    def __init__(self, module_cfg):
        ...
        if self._cfg.embed_diffuse_mask:
            total_edge_feats += 2
        self.edge_embedder = nn.Sequential(nn.Linear(total_edge_feats, self.c_p), nn.ReLU(), nn.Linear(self.c_p, self.c_p), nn.ReLU(), nn.Linear(self.c_p, self.c_p),
    ...
    def _cross_concat(self, feats_1d, num_batch, num_res):
        return torch.cat([torch.tile(feats_1d[:, :, None, :], (1, 1, num_res, 1)), torch.tile(feats_1d[:, None, :, :], (1, num_res, 1, 1)), ], dim=-1).float().reshape(
            [num_batch, num_res, num_res, -1])

    def forward(self, s, t, p_mask, diffuse_mask, chain_idx):
        # Input: [b, n_res, c_s]
        num_batch, num_res, _ = s.shape
        pair_structure_mask = diffuse_mask[:, :, None] * diffuse_mask[:, None, :]
        ...
        all_edge_feats = torch.concat(all_edge_feats, dim=-1) * pair_structure_mask.unsqueeze(-1)
