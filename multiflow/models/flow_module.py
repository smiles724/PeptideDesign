...
from multiflow.data import torus
...
class FlowModule(LightningModule):
    ...
    def model_step(self, noisy_batch: Any):
        ...
        pred_logits_ipa = model_output['pred_logits_ipa']  # (B, N, aatype_pred_num_tokens)
        ...
        angle_loss = torch.sum(((gt_angle_vf_vec - pred_angle_vf_vec) * ang_norm_scale) ** 2 * angle_mask, dim=(-1, -2)) / (
                torch.sum(angle_mask, dim=(-1, -2)) + 1e-8)   # (B,)
        angle_loss *= training_cfg.torsion_loss_weight

        # seqs vf loss
        pred_logits = model_output['pred_logits']
        aatypes_loss_llm = F.cross_entropy(pred_logits.view(-1, pred_logits.shape[-1]), gt_aatypes_1.view(-1), reduction='none').view(pred_logits.shape[:-1]) / cat_norm_scale
        ...
        ce_loss = F.cross_entropy(pred_logits_ipa.reshape(-1, self.aatype_pred_num_tokens), gt_aatypes_1.flatten().long(), reduction='none').reshape(num_batch,
                                                                                                                                                     num_res) / cat_norm_scale
        ...
        train_loss = trans_loss + rots_vf_loss + auxiliary_loss + aatypes_loss + angle_loss + aatypes_loss_llm
        if torch.any(torch.isnan(train_loss)):
            raise ValueError('NaN loss encountered')
        return {"trans_loss": trans_loss, "auxiliary_loss": auxiliary_loss, "rots_vf_loss": rots_vf_loss, "train_loss": train_loss, 'aatypes_loss': aatypes_loss,
                'angles_loss': angle_loss, 'aatypes_loss_llm': aatypes_loss_llm}
    ...
    def compute_sample_metrics(self, model_traj, bb_traj, aa_traj, clean_aa_traj, true_bb_pos, true_aa, diffuse_mask, sample_id, sample_dir, aatypes_corrupt,
