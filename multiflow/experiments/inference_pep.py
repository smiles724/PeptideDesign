...
from multiflow.experiments.utils import seed_all, recursive_to
from multiflow.models.flow_model_pep import PepModel
from multiflow.data.torsion import full_atom_reconstruction, get_heavyatom_mask
from multiflow.data.writers import save_pdb
...
def main(cfg: DictConfig):
    ...
    save_path = os.path.join(sam_cfg.output, '{}_{}_{}'.format(sam_cfg.ckpt_path, sam_cfg.num_steps, sam_cfg.num_samples) + 'deterministic')
    if sam_cfg.x_mirror:
        save_path += '_x_mirror'
    ...
    for i in tqdm(range(len(dataset))):
        try:
            ...
            batch = recursive_to(collate_fn(data_list), device)     # TODO: use esm or IPA for inference?
            name = batch['id'][0]
            traj_1 = model.sample(batch, num_steps=sam_cfg.num_steps)
            ...
            print(f'tran:{ca_dist:.3f} | rot:{rot_dist:.3f} | best_aar: {best_aar:.3f} | aar:{avg_aar:.3f} | len:{pep_len:.0f}')  # average metrics
        ...
        # data_saved = {'chain_nb': batch['chain_nb'][0], 'chain_id': chain_id, 'resseq': batch['resseq'][0], 'icode': icode, 'aa': batch['aa'][0],
        #               'mask_heavyatom': batch['mask_heavyatom'][0], 'pos_heavyatom': batch['pos_heavyatom'][0], }
        # save_pdb(data_saved, path=f'{save_path}/{name}/gt.pdb')
